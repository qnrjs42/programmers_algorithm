...
function solution(cards1, cards2, goal) {
  ...
  for (let i = 0; i < goal.length; i++) {
    if (goal[i] === cards1[0]) {
      cards1.shift();
      comparses.push(goal[i]);
    } else if (goal[i] === cards2[0]) {
      cards2.shift();
      comparses.push(goal[i]);
    }
  }
  return comparses.join('') === goal.join('') ? 'Yes' : 'No';
}
...
console.log(
  solution(['i', 'sadness', 'sad'], ['tired'], ['i', 'sadness', 'sad', 'tired'])
);
console.log(
  solution(
    ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'],
...
// 다른 사람의 풀이
function anotherSolution(cards1, cards2, goal) {
  let j = 0;
  let k = 0;
  for (let i = 0; i < goal.length; i++) {
